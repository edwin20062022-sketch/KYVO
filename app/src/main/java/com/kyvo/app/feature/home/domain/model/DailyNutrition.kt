...
data class MealItem(val id: String, val name: String, val quantity: Double, val unit: String, val calories: Int, val protein: Int, val carbohydrates: Int, val fat: Int, val image: String? = null, val fiber: Double? = null, val sugar: Double? = null, val sodiumMg: Double? = null, val grams: Double? = null, val foodId: String? = null, val foodType: FoodType? = null)
enum class MealType(val label: String) { Breakfast("Desayuno"), Lunch("Comida"), Dinner("Cena"), Snack("Snack") }

fun Meal.withCalculatedTotals(): Meal = copy(
    totalCalories = items.sumOf(MealItem::calories), protein = items.sumOf(MealItem::protein),
    carbohydrates = items.sumOf(MealItem::carbohydrates), fat = items.sumOf(MealItem::fat),
)

fun NutritionPlan.toDailyNutrition(date: LocalDate, meals: List<Meal>): DailyNutrition {
    val normalizedMeals = meals.map(Meal::withCalculatedTotals)
    val calories = normalizedMeals.sumOf(Meal::totalCalories)
    return DailyNutrition(date, targetCaloriesKcal, calories, max(targetCaloriesKcal - calories, 0), proteinGrams, normalizedMeals.sumOf(Meal::protein), carbohydrateGrams, normalizedMeals.sumOf(Meal::carbohydrates), fatGrams, normalizedMeals.sumOf(Meal::fat), normalizedMeals)
}
