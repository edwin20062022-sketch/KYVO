...
fun SettingsScreen(session: AuthSession, onBack: () -> Unit = {}, onProfile: () -> Unit = {}, onAccount: () -> Unit = {}, onFuture: (String) -> Unit = {}, onSignOut: () -> Unit = {}) {
    SettingsScaffold(SETTINGS_SCREEN_TAG, "Ajustes", "Personaliza tu experiencia en KYVO", onBack, showBrand = true) {
        ...
        item { SettingsSection("PREFERENCIAS", listOf(SettingRowData("Centro de notificaciones", "Novedades y avisos importantes", Icons.Outlined.Info) { onFuture("Centro de notificaciones") }, SettingRowData("Unidades", "Sistemas de medida", Icons.Outlined.Info) { onFuture("Unidades") }, SettingRowData("Notificaciones", "Recordatorios y avisos", Icons.Outlined.Info) { onFuture("Notificaciones") }, SettingRowData("Apariencia", "Modo claro u oscuro", Icons.Outlined.Info) { onFuture("Apariencia") }, SettingRowData("Meal Share", "Configuración de plantillas y compartir", Icons.Outlined.Info) { onFuture("Preferencias de Meal Share") })) }
